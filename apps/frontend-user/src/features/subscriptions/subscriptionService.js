...
export const subscriptionApi = {
	...
	order: (orderCode) =>
		api.get(`/v1/me/subscriptions/orders/${encodeURIComponent(orderCode)}`).then(unwrap),
	purchase: (packageId, payerName, payerAccountLast3) =>
		api.post("/v1/me/subscriptions/purchase", {
			packageId,
			payerName,
			payerAccountLast3,
		}).then(unwrap),
