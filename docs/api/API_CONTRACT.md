...
The canonical prefix for versioned business endpoints is `/api/v1`. Existing `/api/auth`, `/api/users/me`, and `/api/admin` routes are compatibility contracts, not examples for new modules. They remain until every user, admin, bot, and external caller has migrated and compatibility tests or telemetry provide removal evidence.
